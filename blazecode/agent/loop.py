...
from blazecode.agent.tool_events import execute_tool, resolve_tool_name, tool_call_message, tool_state
...
class AgentLoop:
    ...
    def _finish(self, text: str, error: str | None, state: State) -> str:
        self._state(state)
        if error and state is not State.SUCCESS:
            if state is State.ERROR or error == "interrupted":
    ...
    async def _run_tool(self, call: ToolCallStart) -> None:
        ...
        label = tool.name if tool is not None else call.name
    ...
    def _api_messages(self, skill_prompt: str) -> list[dict[str, Any]]:
        ...
        compacted = compact_messages([system, *self.messages], maximum)
