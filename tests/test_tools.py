"""Tests for the six tools and the tool registry."""
...
def test_read_basic(tmp_cwd: Path) -> None:
...
def test_read_nonexistent(tmp_cwd: Path) -> None:
    out = _run(ReadTool().run(path="does_not_exist.txt"))
    assert "error" in out.lower()


def test_read_line_range(tmp_cwd: Path) -> None:
...
def test_read_truncates_large_file(tmp_cwd: Path) -> None:
    big = "\n".join(f"x{i}" for i in range(5000))
    (tmp_cwd / "big.txt").write_text(big)
    out = _run(ReadTool().run(path="big.txt"))
    assert "truncated" in out


def test_write_creates_file(tmp_cwd: Path) -> None:
    out = _run(WriteTool().run(path="out/new.txt", content="hello\nworld\n"))
    assert "wrote" in out
    assert (tmp_cwd / "out" / "new.txt").read_text() == "hello\nworld\n"


def test_write_overwrites(tmp_cwd: Path) -> None:
    p = tmp_cwd / "f.txt"
    p.write_text("old")
...
def test_edit_unique_match(tmp_cwd: Path) -> None:
    p = tmp_cwd / "f.py"
    p.write_text("def hello():\n    return 1\n")
    out = _run(
        EditTool().run(path=str(p), old_string="return 1", new_string="return 42")
    )
    assert "edited" in out
    assert p.read_text() == "def hello():\n    return 42\n"


def test_edit_no_match(tmp_cwd: Path) -> None:
    p = tmp_cwd / "f.py"
    p.write_text("foo\nbar\n")
...
def test_edit_multiple_matches(tmp_cwd: Path) -> None:
    p = tmp_cwd / "f.py"
    p.write_text("x = 1\nx = 1\n")
...
def test_edit_missing_file(tmp_cwd: Path) -> None:
    out = _run(EditTool().run(path="nope.txt", old_string="a", new_string="b"))
    assert "not found" in out.lower()


def test_glob_matches(tmp_cwd: Path) -> None:
...
def test_glob_respects_gitignore(tmp_cwd: Path) -> None:
...
def test_glob_no_match(tmp_cwd: Path) -> None:
    out = _run(GlobTool().run(pattern="*.zzz"))
    assert "no matches" in out


def test_grep_finds_matches(tmp_cwd: Path) -> None:
...
def test_grep_no_match(tmp_cwd: Path) -> None:
    (tmp_cwd / "a.py").write_text("nothing here\n")
    out = _run(GrepTool().run(pattern="nothingsuchstring"))
    assert "no matches" in out


def test_shell_success(tmp_cwd: Path) -> None:
    out = _run(ShellTool().run(command=f"echo hi > {tmp_cwd/'out.txt'}"))
    assert "exit 0" in out
    assert (tmp_cwd / "out.txt").read_text().strip() == "hi"


def test_shell_nonzero_exit(tmp_cwd: Path) -> None:
    out = _run(ShellTool().run(command="exit 7"))
    assert "exit 7" in out


def test_shell_timeout(tmp_cwd: Path) -> None:
    out = _run(ShellTool().run(command="sleep 5", timeout=1))
    assert "timed out" in out.lower()


def test_shell_empty_command(tmp_cwd: Path) -> None:
...
def test_registry_preview_for_edit(tmp_cwd: Path) -> None:
    p = tmp_cwd / "f.txt"
